...
export class CodeCompilerService {
  ...
  private async executeJava(code: string): Promise<string> {
    try {
      ...
      const executionResult = await this.executeJavaProgram(folderName, className);
  ...
  private compileJava(filePath: string): Promise<{ stdout: string; stderr: string }> {
  ...
  private executeJavaProgram(folderName: string, className: string): Promise<string> {
    return new Promise<string>((resolve, reject) => {
